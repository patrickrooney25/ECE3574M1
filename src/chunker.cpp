...
std::vector<Chunk> Chunker::chunk(const Document& document, std::size_t document_order) const {
    ...
    while(start_token_idx<total_tokens){
        ...
        if(remaining_tokens<=policy_.max_tokens){
            ...
            for (std::size_t i = window_start_idx; i < max_end_idx; i++){
                if (i>start_token_idx && tokens[i].paragraph> tokens[i-1].paragraph){ //tokens index exceeds previous tokens, so boundary is created
